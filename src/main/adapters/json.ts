// src/main/adapters/json.ts —— D1: claude/gemini/opencode 的 JSON 形态 MCP 增删读
...
type JsonKind = 'claude' | 'gemini' | 'opencode'

const KIND_KEY: Record<JsonKind, string> = {
  claude: 'mcpServers',
  gemini: 'mcpServers',
  opencode: 'mcp'
}
...
export async function readJsonMcp(
  ...
  const rawMap = obj[KIND_KEY[kind]]
  ...
  for (const [id, entry] of Object.entries(rawMap as Record<string, unknown>)) {
    ...
    out[id] = kind === 'opencode' ? openCodeToSpec(entry as Record<string, unknown>) : (entry as McpSpec)
  }
  return out
}
...
export async function writeJsonMcpEntry(
  ...
  const obj = await readJsonObject(filePath)
  const map = ensureObjectMap(obj, KIND_KEY[kind])
  map[id] = kind === 'opencode' ? specToOpenCode(spec) : spec
...
export async function removeJsonMcpEntry(
  ...
  const obj = await readJsonObject(filePath)
  const existing = obj[KIND_KEY[kind]]
  if (typeof existing === 'object' && existing !== null && !Array.isArray(existing)) {
